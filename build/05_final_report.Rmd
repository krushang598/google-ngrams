...
year_total %>% filter(year >= 1800,year <= 2012,term==c(1883,1910,1950)) %>% select(year,prop,term) %>% mutate(term=factor(as.character(term))) %>% ggplot() + geom_line(mapping=aes(x=year,y=prop,color=term)) + scale_y_continuous(labels = label_percent()) + xlab("Year") + ylab("Proportion")
```
...
year_total %>% filter(year >= 1800,year <= 2012,term==c(1883,1910,1950)) %>% select(year,volume,term) %>% mutate(term=factor(as.character(term))) %>% ggplot() + geom_line(mapping=aes(x=year,y=volume,color=term)) + scale_y_continuous(labels=comma) + xlab("Year") + ylab("Volume")
```
...
```{r plot-totals}
year_total %>% filter(year >= 1800,year <= 2012) %>% select(year,total_volume) %>% ggplot() + geom_line(mapping=aes(x=year,y=total_volume)) + scale_y_continuous(labels=comma) + xlab("Year") + ylab("Total Volume")
```
...
max_prop <- year_total %>% filter(year >= 1800,year <= 2012) %>% select(term, year, prop) %>% group_by(term) %>% arrange(term, desc(prop)) %>% top_n(n=1, wt=prop) %>% `colnames<-`(c("term", "max_year", "max_prop"))
```
...
half_max_prop <- inner_join(max_prop,year_total,by="term") %>% filter(year >= 1800,year <= 2012) %>% group_by(term) %>% mutate(half=max(prop)/2) %>% ungroup() %>% arrange(term,desc(prop)) %>% filter(prop<half,max_year<year) %>% group_by(term) %>% top_n(1,prop) %>% select(term,max_year,year) %>% mutate(halflife=year-max_year) %>% `colnames<-`(c("term", "max_year", "half_year", "halflife"))
```
